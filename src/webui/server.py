...
def create_app() -> FastAPI:
    ...
    async def save_config(request: TemplateSaveRequest):
        ...
        try:
            ...
            # Update or add template settings
            title_found = False
            message_found = False
            
            for i, line in enumerate(lines):
                if line.startswith("TITLE_TEMPLATE="):
                    lines[i] = f'TITLE_TEMPLATE={request.title_template}'
                    title_found = True
                elif line.startswith("MESSAGE_TEMPLATE="):
                    lines[i] = f'MESSAGE_TEMPLATE={request.message_template}'
                    message_found = True
            ...
            return {"success": True, "message": "Configuration saved. Restart service to apply."}
        except Exception as e:
